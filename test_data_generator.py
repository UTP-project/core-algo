...
play_time = np.round(np.random.rand(g_num + 1) * (play_time_high - play_time_low) + play_time_low, decimals)

time_window = []
for i in range(g_num + 1):
    time_window.append(sorted([round(random.uniform(0, everyday_time), decimals), round(random.uniform(0, everyday_time), decimals)]))
time_window[0] = [0, 24]
